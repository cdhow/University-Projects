...
// Defaults
Point::Point() : Shape(0, 0, '0') {}
Point::Point(int x, int y, char s) : Shape(x, y, s) {}
// Copy Constructor
Point::Point(Point const & that) : Shape(that.x, that.y, that.s) {}
// Move Constructor
Point::Point(Point && that) noexcept : Shape(std::move(that))
{
    that.x = 0;
    that.y = 0;
    that.s = 0;
